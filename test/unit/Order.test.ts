...
test("Deve criar um pedido com 3 itens (com descrição, preço e quantidade)", function(){
    const order = new Order("129.566.540-98");
...
test("Deve criar um pedido com cupom de desconto expirado", function(){
    const order = new Order("129.566.540-98", new Date("2021-03-10T10:00:00"));
    order.addItem(new Item(1, "Guitarra", 1000),1);    
    order.addItem(new Item(2, "Amplificador", 5000),1);    
    order.addItem(new Item(3, "Cabo", 30),3);
...
test("Deve criar um pedido com 3 itens e calcular o frete", function(){
    ...
    order.addItem(new Item(1, "Aplificador", 5000, new Dimension(50, 50, 50), 20), 1);
    order.addItem(new Item(1, "Cabo", 30, new Dimension(10, 10, 10), 1), 3);
    const total = order.getFreight();
